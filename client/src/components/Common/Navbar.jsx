...
const Navbar = () => {
    ...
    try {
      const response = await fetch(`http://localhost:8080/api/cart/${userId}`, {  // Updated URL
        method: "GET",
        headers: {
      ...
      if (!response.ok) {
        ...
        console.error(`Error fetching cart items: ${response.status} - ${errorText}`);
					...
					<div className="relative flex h-16 items-center justify-between">
						...
						{token && (
							<div className="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
								<motion.a
									href="#"
									onClick={handleCartClick} // Click to go to cart page
